...
## note that "echo=FALSE" means the code will not show up in the HTML output; 
## "include=FALSE" means the output object is not in the HTLM
...
#ecoregions <- vect(file.path(main_dir,gis_dir,"terr-ecoregions-TNC", "tnc_terr_ecoregions.shp"))
  # country boundaries
world_countries <- raster::shapefile(file.path(main_dir,gis_dir, "UIA_World_Countries_Boundaries/World_Countries__Generalized_.shp"))
...
#  cat("starting ",Sl,", ",i," of ",length(spp.all),".\n", sep="")
  
  ## read in occurrence records (includes ex situ)
  #spp.pts <- read.csv(file.path(path.pts, paste0(Sl, ".csv")))
  spp.pts <- Od
  ...
  spp.pts.buffer <- create.buffers(spp.pts,50000,"EPSG:4326","EPSG:4326",boundary.poly)
  ...
  if(nrow(ex.pts)>0){
    ...
    if(Sl %in% yes_sdm){
      ...
      #spp.raster <- raster(file.path(path.sdm,paste0(Sl,"_PNAS_2020_SDM.tif")))
      spp.raster <- Rl
  ...
  #  paste0(Sl, "__prelim_map_v2.html")))

...
path.sdm <- file.path(main_dir,"sdm_rasters","ALL_SDM")
...
ecoregions <- vect(file.path(main_dir,gis_dir,"terr-ecoregions-TNC", 
  ...
  # filter to only regions of interest, to make smaller
...
boundary.poly <- aggregate(ecoregions,dissolve = TRUE)
# protected areas raster
protected_areas <- raster(file.path(main_dir,gis_dir,"wdpa_reclass.tif"))
  # clip to ecoregions layer
#protected_areas <- crop(protected_areas, extent(eco_sf))
...
yes_sdm <- unique(gsub("-spdist_thrsld_median.tif","",yes_sdm))
length(yes_sdm) #84
...
for(i in 35:length(spp.all)){
  ...
  vect_buff <- vect(spp.pts.buffer)
  
  ## create subset of ecoregions within buffers
  #eco_sel <- terra::crop(ecoregions,vect_buff)
    # create sf version of for mapping
  #eco_sf <- sf::st_as_sf(eco_sel)
  ## create ecoregion color palette
  #eco.pal <- colorFactor(palette = "Greys", domain = eco_sf$ECO_ID_U,
  #  reverse = F, na.color = "white")

  ## create subset of protected areas within buffers
  #pa_sel <- crop(protected_areas,extent(spp.pts.buffer))
  ...
  if(nrow(ex.pts)>0){
    ...
    if(spp.all[i] %in% yes_sdm){
      ## read in SDM raster
      spp.raster <- raster(file.path(path.sdm,paste0(spp.all[i],"-spdist_thrsld_median.tif")))
        # if there is an ecoregions band too, create version with just those
        # (for clipping ecoregions and protected areas), then remove so just SDM
        spp.raster.ecos <- spp.raster
        spp.raster.ecos[which(spp.raster.ecos[] == 1)] <- 0
        spp.raster[which(spp.raster[] != 1)] <- NA
        # select color for raster when mapped
        raster.pal <- colorNumeric("#0e6187",values(spp.raster),na.color = "transparent")
      ## clip ecoregions and protected areas to sdm ecoregions
      eco_sel <- crop(ecoregions,spp.raster.ecos)
      # sf version of for mapping
      eco_sf <- sf::st_as_sf(eco_sel)
      pa_sel <- crop(protected_areas,spp.raster.ecos)
      ## map everything
      print("mapping everything")
      map <- create.full.map(eco_sf,spp.raster,raster.pal,spp.pts.buffer,spp.pts,
                             ex.pts.buffer,ex.pts,pa_sel)
    } else { # no sdm
      ## map without sdm
    ...
    if(spp.all[i] %in% yes_sdm){
      ...
      map <- create.noex.map(eco_sf,spp.raster,raster.pal,spp.pts.buffer,
                             spp.pts,pa_sel)
